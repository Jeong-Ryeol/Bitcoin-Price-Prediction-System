...
### 1.2 프로젝트 목표
데이터마이닝 기법과 머신러닝 알고리즘을 활용하여 암호화폐 시장의 비트코인 가격 방향을 예측하는 시스템을 개발한다. Upbit Public API를 통해 1년치 시간별 데이터를 수집하고, 기술적 지표를 분석하여 4가지 분류 알고리즘으로 1시간 후 가격 방향을 예측한다. WEKA 스타일의 데이터마이닝 분석과 웹 기반 대시보드를 제공한다.
